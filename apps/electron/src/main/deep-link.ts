 ...
 * Deep Link Handler
 *
 * Parses pmiagent:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   pmiagent://allChats[/chat/{sessionId}]            - Chat list (all chats)
 *   pmiagent://flagged[/chat/{sessionId}]             - Chat list (flagged filter)
 *   pmiagent://state/{stateId}[/chat/{sessionId}]     - Chat list (state filter)
 *   pmiagent://sources[/source/{sourceSlug}]          - Sources list
 *   pmiagent://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   pmiagent://action/{actionName}[/{id}][?params]
 *   pmiagent://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   pmiagent://allChats                               (all chats view)
 *   pmiagent://allChats/chat/abc123                   (specific chat)
 *   pmiagent://settings/shortcuts                     (shortcuts page)
 *   pmiagent://sources/source/github                  (github source info)
 *   pmiagent://action/new-chat                        (uses active window)
 *   pmiagent://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   pmiagent://workspace/ws123/allChats/chat/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., pmiagent://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., pmiagent://allChats/chat/abc → hostname='allChats', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // pmiagent://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // pmiagent://allChats/..., pmiagent://settings/..., etc. (compound routes)
    ...
    // pmiagent://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // pmiagent://action/... (no workspace - uses active window)
    if (host === 'action') {
