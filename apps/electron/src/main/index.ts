...
import { getWorkspaces, loadStoredConfig } from '@pmi-agent/shared/config'
import { initializeDocs } from '@pmi-agent/shared/docs'
import { ensureDefaultPermissions } from '@pmi-agent/shared/agent/permissions-config'
import { ensureToolIcons } from '@pmi-agent/shared/config'
import { setBundledAssetsRoot } from '@pmi-agent/shared/utils'
import { handleDeepLink } from './deep-link'
import { registerThumbnailScheme, registerThumbnailHandler } from './thumbnail-protocol'
import log, { isDebugMode, mainLog, getLogFilePath } from './logger'
import { setPerfEnabled, enableDebug } from '@pmi-agent/shared/utils'
...
// Custom URL scheme for deeplinks (e.g., pmiagent://auth-complete)
// Supports multi-instance dev: CRAFT_DEEPLINK_SCHEME env var (pmiagent1, pmiagent2, etc.)
const DEEPLINK_SCHEME = process.env.CRAFT_DEEPLINK_SCHEME || 'pmiagent'
...
// Supports multi-instance dev: CRAFT_APP_NAME env var (e.g., "PMI Agent [1]")
app.setName(process.env.CRAFT_APP_NAME || 'PMI Agent')

// Register as default protocol client for pmiagent:// URLs
...
app.whenReady().then(async () => {
  ...
  // Seed tool icons to ~/.pmi-agent/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()
