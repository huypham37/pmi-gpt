        ...
        <div>
          ...
          {context?.documents.length === 0 ? (
            <div className="text-xs text-muted-foreground/70 p-3 rounded-lg border border-dashed border-border text-center">
              No documents added yet. Upload PDF or DOCX files to provide additional context.
            </div>
          ) : (
...
function DocumentRow({
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/30">
      ...
      {expanded && document.extractedText && (
        <div className="px-3 pb-3 border-t border-border/50">
          <pre className="text-xs text-muted-foreground whitespace-pre-wrap mt-2 max-h-48 overflow-y-auto">
            {document.extractedText.slice(0, 3000)}
            {document.extractedText.length > 3000 && '\n\n... (truncated)'}
          </pre>
        </div>
      )}
    </div>
