...
const api: ElectronAPI = {
  ...
  createSource: (workspaceId: string, config: Partial<import('@pmi-agent/shared/sources').FolderSourceConfig>) =>
  ...
  onSourcesChanged: (callback: (sources: import('@pmi-agent/shared/sources').LoadedSource[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, sources: import('@pmi-agent/shared/sources').LoadedSource[]) => {
      callback(sources)
    }
  ...
  onSkillsChanged: (callback: (skills: import('@pmi-agent/shared/skills').LoadedSkill[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, skills: import('@pmi-agent/shared/skills').LoadedSkill[]) => {
      callback(skills)
    }
  ...
  onAppThemeChange: (callback: (theme: import('@pmi-agent/shared/config').ThemeOverrides | null) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, theme: import('@pmi-agent/shared/config').ThemeOverrides | null) => {
      callback(theme)
    }
