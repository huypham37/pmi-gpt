...
DOWNLOAD_DIR="$HOME/.pmi-agent/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="PMI-Agent-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.lukilabs.pmi-agent"
    if pgrep -x "PMI Agent" >/dev/null 2>&1; then
        info "Quitting PMI Agent..."
        ...
        if pgrep -x "PMI Agent" >/dev/null 2>&1; then
            ...
            pkill -9 -x "PMI Agent" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  PMI Agent has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'PMI Agent'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.pmi-agent/app"
    WRAPPER_PATH="$INSTALL_DIR/pmi-agents"
    APPIMAGE_INSTALL_PATH="$APP_DIR/PMI-Agent-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "PMI-Agent.*AppImage" >/dev/null 2>&1; then
        info "Stopping PMI Agent..."
        pkill -f "PMI-Agent.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# PMI Agent launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.pmi-agent/app/PMI-Agent-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@pmi-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@pmi-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: PMI Agent not found at $APPIMAGE_PATH"
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/PMI-Agent-x64.AppImage"
