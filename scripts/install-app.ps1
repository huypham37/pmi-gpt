...
$process = Get-Process -Name "PMI Agent" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing PMI Agent..."
...
Write-Info "Adding 'pmi-agents' command to PATH..."

$binDir = "$env:LOCALAPPDATA\PMI Agent\bin"
$cmdFile = "$binDir\pmi-agents.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\PMI Agent\PMI Agent.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'pmi-agents' command)"
} else {
    Write-Success "Command 'pmi-agents' is ready"
}
...
Write-Host ""
Write-Host "  PMI Agent has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: pmi-agents (restart terminal first)"
Write-Host ""
