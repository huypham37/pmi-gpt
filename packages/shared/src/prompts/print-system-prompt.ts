#!/usr/bin/env bun
/**
 * Debug script to print the full PMI Agent system prompt with annotations.
...
printAnnotation('  2. PMI Agent Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.pmi-agent/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@pmi-agent/electron/main.log' },
  '/Users/example/.pmi-agent/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.pmi-agent/workspaces/abc123/sessions/260121-example-session/plans',
});
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// PMI Agent additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. PMI Agent Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/pmi-agent.ts       ${colors.dim}// User message building${colors.reset}
