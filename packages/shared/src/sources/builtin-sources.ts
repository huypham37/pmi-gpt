 ...
 *
 * NOTE: pmi-agents-docs is now an always-available MCP server configured
 * directly in pmi-agent.ts, not a source. This file is kept for backwards
 ...
 *
 * Currently returns empty array - pmi-agents-docs has been moved to
 * an always-available MCP server in pmi-agent.ts.
 *
...
/**
 * Get the built-in PMI Agent docs source.
 *
 * @deprecated pmi-agents-docs is now an always-available MCP server
 * configured directly in pmi-agent.ts. This function is kept for
...
export function getDocsSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const placeholderConfig: FolderSourceConfig = {
    id: 'builtin-pmi-agents-docs',
    name: 'PMI Agent Docs',
    slug: 'pmi-agents-docs',
    enabled: false,
    provider: 'mintlify',
    type: 'mcp',
    mcp: {
    ...
    tagline: 'Search PMI Agent documentation and source setup guides',
    icon: '📚',
 ...
 *
 * Returns false - pmi-agents-docs is now an always-available MCP server,
