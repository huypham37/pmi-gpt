 ...
 *
 * Stores credentials in an encrypted file at ~/.pmi-agent/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.pmi-agent');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('pmi-agent-v2') // Bumped version for new key derivation
      .digest();
