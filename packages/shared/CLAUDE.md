...
`@pmi-agent/shared` is the core business logic package for PMI Agent. It contains:
...
```typescript
import { CraftAgent, getPermissionMode, setPermissionMode } from '@pmi-agent/shared/agent';
import { loadStoredConfig, type Workspace } from '@pmi-agent/shared/config';
import { getCredentialManager } from '@pmi-agent/shared/credentials';
import { CraftMcpClient } from '@pmi-agent/shared/mcp';
import { loadWorkspaceSources, type LoadedSource } from '@pmi-agent/shared/sources';
import { loadStatusConfig, createStatus } from '@pmi-agent/shared/statuses';
import { resolveTheme } from '@pmi-agent/shared/config/theme';
...
### CraftAgent (`src/agent/pmi-agent.ts`)
...
- Workspace: `~/.pmi-agent/workspaces/{id}/permissions.json`
- Source: `~/.pmi-agent/workspaces/{id}/sources/{slug}/permissions.json`
...
**Storage:** `~/.pmi-agent/workspaces/{id}/statuses/config.json`
...
**Storage:**
- App: `~/.pmi-agent/theme.json`
- Workspace: `~/.pmi-agent/workspaces/{id}/theme.json`
...
All sensitive credentials (API keys, OAuth tokens) are stored in an AES-256-GCM encrypted file at `~/.pmi-agent/credentials.enc`. The `CredentialManager` provides the API for reading and writing credentials.

### Configuration (`src/config/storage.ts`)
Multi-workspace configuration stored in `~/.pmi-agent/config.json`. Supports:
...
### Sources (`src/sources/`)
Sources are external data connections (MCP servers, APIs, local filesystems). Stored at `~/.pmi-agent/workspaces/{id}/sources/{slug}/` with config.json and guide.md. Types: `mcp`, `api`, `local`, `gmail`.

## Dependencies

- `@pmi-agent/core` - Shared types
