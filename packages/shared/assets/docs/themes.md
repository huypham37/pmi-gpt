...
This guide explains how to customize the visual theme of PMI Agent.

## Overview

PMI Agent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.pmi-agent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.pmi-agent/theme.json` - Override specific colors (app-level)
...
```
~/.pmi-agent/workspaces/{id}/config.json
```
...
Create `~/.pmi-agent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.pmi-agent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.pmi-agent/themes/{name}.json`
...
1. Create `~/.pmi-agent/theme.json` for overrides or `~/.pmi-agent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.pmi-agent/theme.json` for overrides, `~/.pmi-agent/themes/` for presets)
