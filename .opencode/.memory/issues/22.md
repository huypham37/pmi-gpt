...
## Current Step
Full E2E pipeline validated — all 6 steps pass. Ready to commit.

## Architecture

### Flow
1. User: "Create test cases for {attack_vector}"
2. Main process → LMStudio (local, OpenAI-compatible): send WSTG list + attack vector → get relevant entries
3. Main process: augment prompt with WSTG content
4. Main process → OpenCode (ACP): create session + send augmented prompt
5. Parse streamed markdown → TestCase[] via `testcase-parser.ts`

### Key Files
| Layer | File | Role |
|-------|------|------|
| Renderer | `pages/TestCaseGeneratorPage.tsx` | Entry point — calls IPC |
| IPC | `main/ipc.ts` | `TESTCASES_GENERATE` handler |
| Main | `main/sessions.ts` | ACP session creation + prompt |
| ACP | `packages/acp-client/src/ACPSession.ts` | Sends augmented prompt to OpenCode |
| Parser | `shared/testcase-parser.ts` | Markdown → TestCase[] (moved from renderer/utils) |
| Agent | `.opencode/agents/testcase-generator.md` | System prompt for OpenCode |
| NEW | `main/lmstudio.ts` | LMStudio client for WSTG retrieval |
| NEW | `main/wstg-data.ts` | Static OWASP WSTG list (108 entries) |
| NEW | `main/wstg-prompt.ts` | Pure functions: parsing, prompt building |
